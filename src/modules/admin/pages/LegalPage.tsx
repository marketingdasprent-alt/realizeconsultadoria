...
const LegalPage = () => {
  ...
  const fetchClients = async () => {
    ...
    try {
      ...
      const dataWithNumbers = (data || []).map((client, index) => ({
        ...client,
  ...
  const arrearsDetails = useMemo(() => {
    ...
    clients.forEach(c => {
      ...
      if (deadlinePassed && remaining > 0) {
        ...
        const delayedParcels =
          c.legal_installments?.filter((inst: any) => {
        ...
        if (delayedParcels.length > 0) {
          isDelayed = true;
          delayedAmount = delayedParcels.reduce(
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="flex items-center gap-2">
            <h1 className="font-display text-2xl sm:text-3xl font-semibold">Jurídico</h1>
          </div>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <Card
            ...
            onClick={() => setListDialogType('paid')}
          >
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">Resolvidos + Parcelas Pagas</p>
            </CardContent>
          </Card>
          <Card
            className="cursor-pointer hover:bg-muted/50 transition-colors"
            onClick={() => setListDialogType('owed')}
          >
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-lg font-semibold w-full text-left">
                Valores a Dever
              </CardTitle>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">Dívida pendente</p>
            </CardContent>
          </Card>
          <Card
            className="cursor-pointer hover:bg-muted/50 transition-colors"
            onClick={() => setListDialogType('arrears')}
          >
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-lg font-semibold w-full text-left">
                Valores em Atraso
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell>
                          <Badge
                            variant="secondary"
                            className="bg-amber-100 text-amber-800 border-amber-200 dark:bg-amber-900/40 dark:text-amber-400 dark:border-amber-800"
                          >
                            {client.litigation_type || 'Contencioso'}
                          </Badge>
                        </TableCell>
                        <TableCell
                          className={`text-right font-semibold ${isZero ? 'text-green-600 dark:text-green-500' : 'text-red-600 dark:text-red-400'}`}
                        >
          ...
          <DialogContent className="max-w-2xl max-h-[85vh] flex flex-col">
            <DialogHeader>
              <DialogTitle className="text-xl">
                {listDialogType === 'arrears'
                  ? 'Detalhes: Valores em Atraso'
                  : listDialogType === 'owed'
                    ? 'Detalhes: Valores a Dever'
                    : 'Detalhes: Valores Pagos'}
              </DialogTitle>
            </DialogHeader>
                    ...
                    (listDialogType === 'arrears'
                      ...
                      .map((item, i) => (
                        ...
                        >
                          ...
                          <TableCell
                            className={`font-bold whitespace-nowrap ${listDialogType === 'arrears' ? 'text-red-500' : listDialogType === 'owed' ? 'text-blue-500' : 'text-green-500'}`}
                          >
                            {new Intl.NumberFormat('pt-PT', {
                              style: 'currency',
                              currency: 'EUR',
                            ...
                            <div className="flex flex-wrap gap-1">
                              {item.dates.split(', ').map((dateStr, idx) => (
                                <Badge
                                  key={idx}
                                  variant="outline"
                                  className={`${listDialogType === 'arrears' ? 'bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400' : listDialogType === 'owed' ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400' : 'bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400'} border-transparent`}
                                >
                                  {dateStr}
                                </Badge>
                              ))}
