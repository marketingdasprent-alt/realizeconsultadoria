...
import { useAdminTickets, type SupportTicket } from '@/hooks/useAdminTickets';
...
const SupportTicketsPage = () => {
  ...
  const { tickets, isLoading, updateTicketStatus, saveAdminNotes } = useAdminTickets();
  ...
  const handleSaveNotes = async () => {
    ...
    setIsUpdating(true);
    try {
      await saveAdminNotes(selectedTicket, adminNotes);
  ...
  const handleDownloadAttachment = async (attachment: TicketAttachment) => {
    try {
      ...
      const buckets = ['ticket-attachments', 'employee-files', 'documents', 'employees'];
      const cleanPath = attachment.file_path.startsWith('/')
        ? attachment.file_path.substring(1)
        : attachment.file_path;

      for (const bucket of buckets) {
        const { data } = await supabase.storage.from(bucket).createSignedUrl(cleanPath, 300);
        if (data?.signedUrl) {
  ...
  const accessibleDepartments = departments.filter(
    d => isSuperAdmin || userDepartmentIds.includes(d.id)
  );
  ...
  return (
    <>
          ...
          <p className="text-muted-foreground mt-1">
            ...
            {!isSuperAdmin && userDepartmentIds.length > 0 && (
              <span className="ml-2 text-xs">
                ({accessibleDepartments.length} departamento
                {accessibleDepartments.length !== 1 ? 's' : ''})
              </span>
            )}
          </p>
        </div>
            ...
            <div className="flex flex-wrap items-center gap-4">
              ...
              {(() => {
                const uniqueSubjects = Array.from(
                  new Set(accessibleTickets.map(t => t.subject))
                ).sort();
            ...
            <div className="flex items-center gap-3">
              {filteredAndSortedTickets.length > 0 && (
                <label className="flex items-center gap-2 cursor-pointer text-sm text-muted-foreground">
                  <Checkbox
                    checked={
                      selectedTicketIds.size === filteredAndSortedTickets.length &&
                      filteredAndSortedTickets.length > 0
                    }
                      ...
                      {isExpanded && (
                        <div className="border-t border-border p-4 bg-muted/20 space-y-4">
                          ...
                          <div className="flex flex-wrap items-center gap-3 pt-2">
                            ...
                            <Button variant="outline" onClick={() => openDetail(ticket)}>
                              Notas do Admin
                            </Button>

                            <TicketConversationDialog ticketId={ticket.id} />
                          </div>
                        </div>
                      )}
                    </div>
      ...
      <Dialog open={isDetailOpen} onOpenChange={setIsDetailOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Notas da Administração</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              <label className="text-sm font-medium mb-1 block">Notas Privadas</label>
              <Textarea
                value={adminNotes}
                onChange={e => setAdminNotes(e.target.value)}
                placeholder="Adicione notas visíveis para o colaborador..."
                rows={5}
              />
              <p className="text-xs text-muted-foreground mt-2">
                As notas adicionadas aqui ficarão visíveis para o colaborador que abriu o ticket.
              </p>
            </div>
            <div className="flex justify-end gap-3">
              ...
              <Button variant="gold" onClick={handleSaveNotes} disabled={isUpdating}>
      ...
      <AlertDialog open={showPrintModeDialog} onOpenChange={setShowPrintModeDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Selecionou {pendingPrintTickets.length} tickets. Como deseja imprimi-los?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="sm:justify-between mt-6">
