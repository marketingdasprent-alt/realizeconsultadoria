...
serve(async req => {
  ...
  try {
    ...
    if (!employee_id || !new_email) {
      return new Response(JSON.stringify({ error: 'employee_id e new_email são obrigatórios' }), {
        status: 400,
    ...
    const employeeData = employee as unknown as {
      id: string;
      name: string;
      email: string;
      user_id: string | null;
    ...
    if (updateError) {
      ...
      return new Response(JSON.stringify({ error: 'Erro ao atualizar email do colaborador' }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
