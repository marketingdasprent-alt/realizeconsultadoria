...
import { Loader2, CalendarIcon, RefreshCw, Package } from "lucide-react";
...
const EmployeeGeneralTab = ({
  ...
  // Carregar equipamentos associados ao colaborador logado/selecionado
      ...
      <CardContent className="pt-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {/* Secção Equipamentos Atribuídos */}
            {isEditMode && employee && (
              <div className="md:col-span-2 pt-4 border-t">
                ...
                {assignedEquipments.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {assignedEquipments.map((eq: any) => (
                      <div key={eq.id} className="border rounded-md p-3 bg-muted/20 flex flex-col justify-between">
                        <div>
                          <p className="font-medium text-sm flex items-center justify-between">
                            <span>{eq.brand} {eq.model}</span>
                            {eq.equipment_number && <span className="text-[10px] font-mono bg-secondary/80 px-1.5 py-0.5 rounded-sm">#{eq.equipment_number}</span>}
                          </p>
