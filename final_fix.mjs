...
async function finalFix() {
  ...
  if (adminUser || adminId) {
    ...
    await supabase
      .from('profiles')
      .upsert({ user_id: finalId, name: 'Dinis Silva', email: adminEmail });
    console.log('-> @dasprent agora é SUPER ADMIN (pass: dicadica21)');
  } else {
    console.log(
      '-> AVISO: Não consegui localizar o ID do @dasprent. Por favor, crie a conta no site se ainda não o fez.'
    );
  }
