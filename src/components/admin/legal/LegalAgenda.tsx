...
export default function LegalAgenda({ clients }: LegalAgendaProps) {
  ...
  const fetchManualEvents = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        console.warn(
          'Could not fetch agenda events, matching table might not exist yet:',
          error.message
        );
      } else {
      ...
      if (c.legal_installments) {
        c.legal_installments.forEach((inst: any) => {
          if (inst.status === 'Pendente' && inst.due_date) {
            ...
            if (forceStartOfDay(new Date(y, m - 1, d)).getTime() === dTime) {
              evts.push({
                title: `${c.name} (Parcela: ${Number(inst.amount) || 0}€)`,
                type: 'installment',
              });
            }
          }
  ...
  const handleAddManualEvent = async () => {
    ...
    const { error } = await supabase
      .from('legal_agenda_events')
      .insert([{ title: newEventTitle, event_date }]);

    if (error) {
      toast({
        title: 'Erro',
        description:
          "A tabela 'legal_agenda_events' poderá ainda não ter sido criada no Supabase. Execute o script fornecido.",
        variant: 'destructive',
      });
    } else {
        ...
        <CardContent className="p-0">
          <div className="grid grid-cols-7 border-b">
            {weekDays.map(day => (
              <div
                key={day}
                className="text-center py-2 text-sm font-semibold text-muted-foreground border-r last:border-r-0"
              >
                {day}
              </div>
            ))}
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-start">
                    <span
                      className={`text-sm font-medium ${isToday ? 'bg-primary text-primary-foreground w-6 h-6 rounded-full flex items-center justify-center' : ''}`}
                    >
                      {format(day, 'd')}
                    </span>
                  </div>
                  ...
                  <div className="flex flex-col gap-1 mt-1 flex-1 overflow-y-auto w-full no-scrollbar">
                    ...
                    {acts.clientEvts.map((ce, i) => (
                      <div
                        key={i}
                        className="text-[10px] bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400 px-1 py-0.5 rounded border border-red-200 truncate"
                        title={ce.title}
                      >
                        € {ce.title.split(' ')[0]}
                      </div>
                    ...
                    {acts.dbEvents.map((de, i) => (
                      <div
                        key={i}
                        className="text-[10px] bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400 px-1 py-0.5 rounded border border-blue-200 truncate"
                        title={de.title}
                      >
                        {de.title}
                      </div>
          ...
          <div className="space-y-4 py-4">
            {selectedDate &&
              (() => {
                ...
                const isEmpty =
                  !acts.holiday && acts.clientEvts.length === 0 && acts.dbEvents.length === 0;
                ...
                return (
                  <div className="space-y-3">
                    ...
                    {acts.clientEvts.map((c, i) => (
                      <div
                        key={`c-${i}`}
                        className="flex items-center gap-2 p-2 bg-red-50 dark:bg-red-900/20 rounded border border-red-100"
                      >
                    ...
                    {acts.dbEvents.map((d, i) => (
                      <div
                        key={`d-${i}`}
                        className="flex items-center gap-2 p-2 bg-blue-50 dark:bg-blue-900/20 rounded border border-blue-100 group"
                      >
                    ...
                    {isEmpty && (
                      <p className="text-sm text-muted-foreground italic">
                        Não existem eventos associados a este dia.
                      </p>
                    )}
                  </div>
                );
