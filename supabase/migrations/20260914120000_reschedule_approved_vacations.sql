CREATE OR REPLACE FUNCTION public.reschedule_approved_vacation(
  p_absence_id uuid,
  p_periods jsonb,
...
BEGIN
  ...
  IF v_absence.absence_type <> 'vacation' OR v_absence.status <> 'approved' THEN
    RAISE EXCEPTION 'Apenas férias aprovadas podem ser remarcadas por esta operação.';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Escolha datas diferentes das férias atualmente aprovadas.';
  END IF;
  ...
  v_target_status := CASE WHEN v_is_admin THEN 'approved' ELSE 'pending' END;
  ...
  SET start_date = v_start_date,
      ...
      approved_by = CASE WHEN v_is_admin THEN approved_by ELSE NULL END,
      approved_at = CASE WHEN v_is_admin THEN approved_at ELSE NULL END,
...
REVOKE ALL ON FUNCTION public.reschedule_approved_vacation(uuid, jsonb, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.reschedule_approved_vacation(uuid, jsonb, text) TO authenticated;
