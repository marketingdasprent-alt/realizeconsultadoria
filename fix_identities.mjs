...
async function finalSolution() {
  ...
  if (gmailUser) {
    ...
    await supabase
      .from('profiles')
      .upsert({ user_id: gmailUser.id, email: workerEmail, name: 'Dinis Silva (Funcionario)' });
  ...
  if (createError) {
    console.log(
      `-> Erro ao criar: ${createError.message}. Tentando localizar utilizador existente...`
    );
  ...
  if (adminId) {
    ...
    await supabase
      .from('profiles')
      .upsert({ user_id: adminId, name: 'Dinis Silva', email: adminEmail });
    console.log('-> @dasprent promovido a SUPER ADMIN.');
  } else {
    console.error(
      'ERRO CRÍTICO: Não consegui gerar um utilizador para o @dasprent. Por favor, remova o utilizador dinis.silva@dasprent.pt manualmente no painel AUTH > USERS do Supabase e avise-me.'
    );
  }
}
