...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (profileError || !profile) {
      ...
      return new Response(JSON.stringify({ error: 'Perfil de administrador não encontrado' }), {
        status: 404,
        headers: { 'Content-Type': 'application/json', ...corsHeaders },
      });
    }
    ...
    if (authError || !authUser?.user) {
      ...
      return new Response(JSON.stringify({ error: 'Utilizador não encontrado na autenticação' }), {
        status: 404,
        headers: { 'Content-Type': 'application/json', ...corsHeaders },
      });
    }
    ...
    if (!employee) {
      console.log(
        '[repair-mixed-account] No employee linked to this user_id, just restoring admin email'
      );

      // Just restore the admin email
      const { error: restoreError } = await supabaseAdmin.auth.admin.updateUserById(admin_user_id, {
      ...
      return new Response(
        JSON.stringify({
          error:
            ...
            employeeEmail +
            ') já está em uso por outra conta. Primeiro altere o email do colaborador para um email único.',
        }),
        { status: 400, headers: { 'Content-Type': 'application/json', ...corsHeaders } }
      );
    }
